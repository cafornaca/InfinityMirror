...
void loop() {
  ...
  for(int k=0;k<12;k++){//Each k is 1 hour
    pixels.setPixelColor(i, pixels.Color(0,0,200)); //blue for an hour marking
    pixels.show();
    for(int j=0; j<60; j++){//Each j is 1 minute
      //seconds
      if (j!=k){
          pixels.setPixelColor(i, pixels.Color(100,0,200)); // bluish purple color for minute and hour
          pixels.show();
        } else{  
          pixels.setPixelColor(i, pixels.Color(200,0,200)); //creates purple light for minutes
          pixels.show();
        }
      ...
      if (j!=k){//will only turn off pixel if minute and hour aren't overlapping
          pixels.setPixelColor(i, pixels.Color(0,0,0)); //This will never have any other color showing in it
          pixels.show();
